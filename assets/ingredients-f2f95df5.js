import{i as u,d as k,a as f}from"./header-e4588436.js";const p=document.querySelector("#modal-cocktail"),S=document.querySelector(".modal");document.querySelector(".modal-cocktail__content");const y=document.querySelector(".modal-ingredients__content"),i="favoriteIngredients",l=document.querySelector(".add-to-favorite-list"),I=JSON.parse(localStorage.getItem(i))??[],c=async(e,n)=>{const r=document.querySelector(".add-to-favorite-list");if(e.length===0)r.innerHTML=`
    ...
    `;else{n.innerHTML="";const a=e.map(t=>`
           ...
           <div class="favorite-ingridient-content">
            ...
            `).join("");n.innerHTML=a}},s=e=>{const n=e.target.dataset.removeid,a=JSON.parse(localStorage.getItem(i)).findIndex(({_id:o})=>o===n);u.splice(a,1),localStorage.setItem(i,JSON.stringify(u));const t=JSON.parse(localStorage.getItem(i));c(t,l).then(()=>{document.querySelectorAll("[data-remove-ingredient]").forEach(m=>m.addEventListener("click",s))}),document.querySelectorAll("[data-learnmore-ingredient]").forEach(o=>o.addEventListener("click",g))};c(I,l).then(()=>{document.querySelectorAll("[data-remove-ingredient]").forEach(n=>n.addEventListener("click",s))});const C=document.querySelectorAll("[data-learnmore-ingredient]"),g=e=>{const n=e.target.dataset.ingredient;k(n).then(()=>{const r=document.querySelector("[data-modal-add-ingredients]");p.style.display="block",y.style.display="block";const a=document.querySelector("[data-modal-close-ingredients]"),t=o=>{o.preventDefault(),p.style.display="none",f()};a.addEventListener("click",t),S.classList.add("modal-ingredient");const d=()=>{r.removeEventListener("click",d),f();const o=JSON.parse(localStorage.getItem(i))??[];c(o,l).then(()=>{document.querySelectorAll("[data-remove-ingredient]").forEach(h=>h.addEventListener("click",s))}),document.querySelectorAll("[data-learnmore-ingredient]").forEach(v=>v.addEventListener("click",g))};r.addEventListener("click",d)})};C.forEach(e=>e.addEventListener("click",g));
